...
| Version | Key Changes |
|---|---|
| 0.2.33 | Proactive knowledge display: (1) web search/wiki MCP tools always injected as candidates when keyword scoring selects no tools — AI can now search for any factual question; (2) knowledge card synthesized from Q+A for ALL general answers even without web search; (3) streaming path now captures and pushes knowledge_card immediately when meta event arrives; (4) VoiceStateOverlay auto-opens source URL iframe when show_url present (no click needed), auto-closes after 60s, clears on next wake word |
