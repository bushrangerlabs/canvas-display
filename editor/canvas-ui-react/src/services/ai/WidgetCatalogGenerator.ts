...
export function generateWidgetCatalog(): string {
  ...
  sections.push('Presentation: opacity(num 0-1), rotation(num degrees), overflow(enum)[hidden|visible|auto], transitionDuration(num ms)');
  sections.push('Accessibility: ariaLabel(text). Give every interactive or status widget a concise purpose.');
  sections.push('');
  ...
  for (const widgetType of widgetNames) {
    ...
    // Add widget section
    const aliases = metadata.aliases?.length ? ` aliases=[${metadata.aliases.join('|')}]` : '';
    sections.push(`${widgetType}: ${metadata.description}${aliases}`);
    if (metadata.defaultSize) {
      sections.push(`  recommendedSize: ${metadata.defaultSize.w}x${metadata.defaultSize.h}`);
    }
    for (const hint of metadata.aiHints ?? []) {
      sections.push(`  // ${hint}`);
    }
