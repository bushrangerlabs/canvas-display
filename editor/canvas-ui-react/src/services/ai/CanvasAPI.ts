...
import { resolveWidgetType, WIDGET_REGISTRY } from '../../shared/registry/widgetRegistry';
...
export class CanvasAPI {
  ...
  private applyDefaults(type: string, config: Record<string, any>): Record<string, any> {
    ...
    metadata.fields.forEach(field => {
      ...
      if ((field.type === 'number' || field.type === 'slider') && typeof value === 'string' && value.trim() !== '') {
        ...
        if (Number.isFinite(numeric)) normalized[field.name] = numeric;
      }
      if (field.type === 'checkbox' && typeof value === 'string') {
        if (value.toLowerCase() === 'true') normalized[field.name] = true;
        if (value.toLowerCase() === 'false') normalized[field.name] = false;
      }
      if (typeof normalized[field.name] === 'number') {
        if (field.min !== undefined) normalized[field.name] = Math.max(field.min, normalized[field.name]);
        if (field.max !== undefined) normalized[field.name] = Math.min(field.max, normalized[field.name]);
      }
    });
  ...
  addWidget(
    ...
    const metadata = resolvedType ? this.getWidgetMetadata(resolvedType) : null;
    if (!metadata) {
    ...
    const finalPosition = {
      x: position?.x ?? 0,
      y: position?.y ?? 0,
      width: Math.max(metadata.minSize?.w ?? 1, Math.min(metadata.maxSize?.w ?? Infinity, position?.width ?? defaultSize.w)),
      height: Math.max(metadata.minSize?.h ?? 1, Math.min(metadata.maxSize?.h ?? Infinity, position?.height ?? defaultSize.h)),
      zIndex: 1,
    };
  ...
  /**
   * Merge configuration keys without replacing the rest of a widget config.
   * This is the safest primitive for iterative AI edits.
   */
  patchWidgetConfig(widgetId: string, changes: Record<string, any>): void {
    if (!this.context.currentViewId) throw new Error('No active view');
    const view = this.context.getView(this.context.currentViewId);
    ...
    this.updateWidget(widgetId, {
      config: this.applyDefaults(widget.type, { ...widget.config, ...changes }),
    });
  }

  /**
   * Resize and move a widget while preserving its other position properties.
   */
  setWidgetFrame(
    widgetId: string,
    frame: Partial<WidgetConfig['position']>,
  ): void {
    if (!this.context.currentViewId) throw new Error('No active view');
    const widget = this.context.getView(this.context.currentViewId)?.widgets.find(item => item.id === widgetId);
    if (!widget) throw new Error(`Widget not found: ${widgetId}`);
    const metadata = this.getWidgetMetadata(widget.type);
    const width = Math.max(metadata?.minSize?.w ?? 1, Math.min(metadata?.maxSize?.w ?? Infinity, frame.width ?? widget.position.width));
    const height = Math.max(metadata?.minSize?.h ?? 1, Math.min(metadata?.maxSize?.h ?? Infinity, frame.height ?? widget.position.height));
    this.updateWidget(widgetId, {
      position: {
        ...widget.position,
        ...frame,
        x: Math.max(0, frame.x ?? widget.position.x),
        y: Math.max(0, frame.y ?? widget.position.y),
        width,
        height,
      },
    });
