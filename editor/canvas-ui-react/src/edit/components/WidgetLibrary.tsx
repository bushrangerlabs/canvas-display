...
export const WidgetLibrary: React.FC<WidgetLibraryProps> = ({ 
  ...
  const categoryWidgets = category === 'all'
    ? allWidgets 
    : allWidgets.filter(w => w.category === category);
  const query = search.trim().toLowerCase();
  const filteredWidgets = query
    ? categoryWidgets.filter(widget =>
        [widget.name, widget.description, widget.type, ...widget.aliases, ...widget.tags]
    ...
    >
      <Box sx={{ p: 2, display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <TextField
          ...
          placeholder="Search widgets, aliases, or uses…"
          size="small"
          fullWidth
          sx={{ mb: 2, flexShrink: 0 }}
          inputProps={{ 'aria-label': 'Search widget library' }}
        />
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 2 }}>
                      ...
                      <Box sx={{ flex: 1 }}>
                        ...
                        <Box sx={{ display: 'flex', gap: 0.75, mt: 1, flexWrap: 'wrap' }}>
                          ...
                          <Chip size="small" label={`${widget.defaultSize.w}×${widget.defaultSize.h}`} variant="outlined" />
                        </Box>
                      </Box>
                    </Box>
                  </CardContent>
