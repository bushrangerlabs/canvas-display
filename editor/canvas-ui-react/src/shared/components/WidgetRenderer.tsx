...
export const WidgetRenderer: React.FC<WidgetRendererProps> = ({
  ...
  const containerStyle: React.CSSProperties = {
    ...
    opacity: isResizing ? 0.7 : (widget.config.opacity ?? widget.config.style?.opacity ?? 1),
    transition: isDraggingManually || isResizing
      ? 'none'
      : `opacity ${widget.config.transitionDuration ?? 180}ms ease, box-shadow ${widget.config.transitionDuration ?? 180}ms ease`,
  ...
  return (
    ...
    >
      ...
      <div
        role="group"
        aria-label={widget.config.ariaLabel || widget.name || widget.type}
        ...
        style={{
          ...
          width: '100%',
          height: '100%',
          overflow: widget.config.overflow ?? 'hidden',
